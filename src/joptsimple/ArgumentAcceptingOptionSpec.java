...
public abstract class ArgumentAcceptingOptionSpec<V> extends
	...
	ArgumentAcceptingOptionSpec(Collection<String> options,
			boolean argumentRequired, String description) {
	...
	public final ArgumentAcceptingOptionSpec<V> withValuesSeparatedBy(
			char separator) {
		if (separator == 0) {
	...
	protected abstract void detectOptionArgument(
			OptionParser paramOptionParser, ArgumentList paramArgumentList,
			OptionSet paramOptionSet);

	protected final V convert(String argument) {
		return (V) convertWith(this.converter, argument);
	}

	protected boolean canConvertArgument(String argument) {
		StringTokenizer lexer = new StringTokenizer(argument,
				this.valueSeparator);
		try {
			while (lexer.hasMoreTokens())
	...
	protected boolean isArgumentOfNumberType() {
		return (this.converter != null)
				&& (Number.class.isAssignableFrom(this.converter.valueType()));
	}
...
/*
 * Location:
 * M:\AFolderOfMinecraft\Minecraft.jar!\joptsimple\ArgumentAcceptingOptionSpec
