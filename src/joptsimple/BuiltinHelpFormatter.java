...
public class BuiltinHelpFormatter implements HelpFormatter {
	...
	public BuiltinHelpFormatter(int desiredOverallWidth,
		...
		this.optionRows = new Rows(desiredOverallWidth,
	...
	public String format(Map<String, ? extends OptionDescriptor> options) {
		Comparator<OptionDescriptor> comparator = new Comparator<OptionDescriptor>() {
			public int compare(OptionDescriptor first, OptionDescriptor second) {
				return ((String) first.options().iterator().next())
						.compareTo((String) second.options().iterator().next());
			}
		};
	...
	private void addNonOptionsDescription(
			Collection<? extends OptionDescriptor> options) {
	...
	private boolean shouldShowNonOptionArgumentDisplay(
	...
	private void maybeAppendNonOptionsDescription(StringBuilder buffer,
			OptionDescriptor nonOptions) {
		buffer.append(
				(buffer.length() > 0)
						&& (!Strings.isNullOrEmpty(nonOptions.description())) ? " -- "
						: "").append(nonOptions.description());
	}

	private OptionDescriptor findAndRemoveNonOptionsSpec(
			Collection<? extends OptionDescriptor> options) {
		for (Iterator<? extends OptionDescriptor> it = options.iterator(); it
				.hasNext();) {
	...
	private boolean hasRequiredOption(
			Collection<? extends OptionDescriptor> options) {
	...
	private void addOptions(Collection<? extends OptionDescriptor> options) {
		for (OptionDescriptor each : options) {
			if (!each.representsNonOptions())
				this.optionRows.add(createOptionDisplay(each),
						createDescriptionDisplay(each));
		}
	}
	...
	private void maybeAppendOptionInfo(StringBuilder buffer,
		...
		if ((indicator != null) || (!Strings.isNullOrEmpty(description)))
			appendOptionHelp(buffer, indicator, description,
	...
	private String extractTypeIndicator(OptionDescriptor descriptor) {
		...
		if ((!Strings.isNullOrEmpty(indicator))
				&& (!String.class.getName().equals(indicator))) {
			return Classes.shortNameOf(indicator);
		}
		return null;
	}

	private void appendOptionHelp(StringBuilder buffer, String typeIndicator,
	...
	private void appendTypeIndicator(StringBuilder buffer,
			String typeIndicator, String description, char start, char end) {
	...
	private String createDescriptionDisplay(OptionDescriptor descriptor) {
		...
		return (descriptor.description() + ' ' + Strings.surround(
				new StringBuilder().append("default: ")
						.append(defaultValuesDisplay).toString(), '(', ')'))
				.trim();
	}
...
/*
 * Location:
 * M:\AFolderOfMinecraft\Minecraft.jar!\joptsimple\BuiltinHelpFormatter.class
