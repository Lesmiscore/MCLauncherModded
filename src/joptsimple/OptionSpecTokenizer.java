...
class OptionSpecTokenizer {
	...
	AbstractOptionSpec<?> next() {
		...
		String optionCandidate = String.valueOf(this.specification
				.charAt(this.index));
		this.index += 1;
		...
		if (hasMore()) {
			...
			spec = hasMore() && (this.specification.charAt(this.index) == ':') ? handleArgumentAcceptingOption(optionCandidate)
...
/*
 * Location:
 * M:\AFolderOfMinecraft\Minecraft.jar!\joptsimple\OptionSpecTokenizer.class
