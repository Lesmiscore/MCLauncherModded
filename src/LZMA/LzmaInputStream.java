...
public class LzmaInputStream extends FilterInputStream {
	...
	byte[] dictionary;
	int dictionarySize, dictionaryPos, GlobalPos, rep0, rep1, rep2, rep3, lc,
			lp, pb, State, uncompressed_size, uncompressed_offset, RemainLen;
	boolean PreviousIsMatch;
	int[] probs;
	byte[] uncompressed_buffer;
	long GlobalNowPos, GlobalOutSize;
	...
	private void LzmaDecode(int paramInt) throws IOException {
		...
		while ((this.RemainLen > 0) && (this.uncompressed_size < paramInt)) {
			...
			this.uncompressed_buffer[(this.uncompressed_size++)] = (this.dictionary[this.dictionaryPos] = this.dictionary[m]);
		...
		while (this.uncompressed_size < paramInt) {
			...
			if (this.RangeDecoder.BitDecode(this.probs, 0 + (this.State << 4)
				...
				this.uncompressed_buffer[(this.uncompressed_size++)] = (byte) i;

				this.dictionary[this.dictionaryPos] = (byte) i;
				if (++this.dictionaryPos == this.dictionarySize) {
					this.dictionaryPos = 0;
				}
			} else {
...
/*
 * Location: M:\AFolderOfMinecraft\Minecraft.jar!\LZMA\LzmaInputStream.class
 * Java compiler version: 4 (48.0) JD-Core Version: 0.7.1
 */